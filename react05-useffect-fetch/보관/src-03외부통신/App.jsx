...
function MyCommunication(props) {
  /*외부 api를 얻어오기 위해 스테이트를 생성한다. 초기값은 json의 포맷에 따라 달라질 수 있으므로
  확인 후 설정한다.*/
  var[myJSON,setmyJSON] = useState({results:[]});
  //해당 컴포넌트의 1차 렌더링 후 api의 정보를 얻어온다.
  useEffect(function() {
    //api서버에서 10명의 정보를 json으로 인출한다.
  ...
  for(let i=0; i<myJSON.results.length; i++) {
    ...
    trTag.push(
      ...
      <tr key={data.login.md5}>
        ...
        <td>{data.login.title} {data.login.first} {data.login.last}</td>
        <td>{data.nat}</td>
        <td>{data.email}</td>
      </tr>
  ...
  return (
    <div className="App">
      ...
      <MyCommunication onProfile={(sData)=>{
        console.log(sData);
        /*백틱 기호를 사용하는 탬플릿 리터럴을 통해 줄바꿈 시 +기호 없이 문자열을 반환한다.*/ 
        let info = `전화번호:${sData.cell} 성별:${sData.gender} usernmae${sData.login.username} password${sData.login.password}`;
        alert(info);
      }}></MyCommunication>
    </div>
